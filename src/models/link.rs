...
pub struct Link {
    ...
    /// The page theme template this link's styling was applied from (UUID).  Empty when the theme was styled by hand rather than taken from a template. Set when a template is applied, and unchanged by later edits, so it records where the look came from rather than what it is now.
