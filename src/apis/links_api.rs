...
/// struct for typed errors of method [`links_service_create_thank_you_link`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LinksServiceCreateThankYouLinkError {
    DefaultResponse(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`links_service_get_lead_score`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum LinksServiceGetLeadScoreError {
    DefaultResponse(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
/// This endpoint allows users to create a new link with a thank you page as the exit step.
pub async fn links_service_create_thank_you_link(configuration: &configuration::Configuration, create_thank_you_link_request: models::CreateThankYouLinkRequest) -> Result<models::CreateThankYouLinkResponse, Error<LinksServiceCreateThankYouLinkError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_create_thank_you_link_request = create_thank_you_link_request;

    let uri_str = format!("{}/v1/links/thank-you", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = req_builder.json(&p_body_create_thank_you_link_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CreateThankYouLinkResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CreateThankYouLinkResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<LinksServiceCreateThankYouLinkError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn links_service_get_lead_score(configuration: &configuration::Configuration, link_id: &str, event_id: &str) -> Result<models::GetLeadScoreResponse, Error<LinksServiceGetLeadScoreError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_link_id = link_id;
    let p_path_event_id = event_id;

    let uri_str = format!("{}/v1/links/{linkId}/lead-score/{eventId}", configuration.base_path, linkId=crate::apis::urlencode(p_path_link_id), eventId=crate::apis::urlencode(p_path_event_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::GetLeadScoreResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::GetLeadScoreResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<LinksServiceGetLeadScoreError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
